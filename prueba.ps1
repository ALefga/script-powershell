...
if ($confirmation -eq "si") {
    # Desconecta las sesiones de los usuarios en VM-CO03
    foreach ($user in $usersToDisconnectCO03) {
        $sessionId = $user.SesionId
        $computer = $user.computadora
        Write-Host "Desconectando sesión de $($user.nombre) con ID de sesión $sessionId en $computer..."
        logoff $sessionId /server:$computer
        Write-Host "Sesión desconectada correctamente."
    }
