...
# ------------------------------------------------------------------------
# Ruta donde se encuentran las scripts que ejecuta el servicio 
#
[string] $eu_PathScript = "C:\Program Files\WindowsPowerShell\eu_scripts"
# ------------------------------------------------------------------------
# Global settings
$serviceName = $script                  # A one-word name used for net start commands
$serviceDisplayName = "Eurotronic: script de powershell"
$ServiceDescription = "Scripts de PS ubicadas en $eu_PathScript que se ejecunta mediante este servicio. Desarrollado por Eurotronic Consultores SL."
...
#                   NewFileg : boolean $true / $false                         #
#                                                                             #
#   Notes           Prefixes the string with a timestamp and the user name.   #
#                   (Except if the string is empty: Then output a blank line.)#
#                   Se borra el fichero al iniciar el servicio                #
#-----------------------------------------------------------------------------#

Function Log () {
  Param(
    [Parameter(Mandatory=$false, ValueFromPipeline=$true, Position=0)]
    [String]$string, # string para el log

    [Parameter(Mandatory=$false, Position=1)] [ValidateSet($false, $true)]
    [boolean]$NewFile = $false # new log: yes/no
...
if ($SCMStart) {                # The SCM tells us to start the service
  ...
  Log "$scriptName -SCMStart: Starting script '$scriptFullName' -Service" -NewFile $True
...
if ($Service) {                 # Run the service
  ...
  try {
    ...
    $timerName = "service timer for $script"
    $period = 10 # seconds
    ...
    do { # Keep running until told to exit by the -Stop handler
      ...
      switch ($message) {
        ...
        # ------ bloque de código a ejecutar cada cierto tiempo -------
        ...
        "TimerTick" { # Example. Periodic event generated for this example
         ...
         # Ejemplo de tarea a ejecutar:
         # Move-Item -Path "c:\temp\prueba.ini" -Destination "c:\temp\temp" -Force

          # 1) --- EJECUTAR CADA 1 HORA; 3600 s = 360 tick
          if (($eu_contador10 % 10 ) -eq 0 ) {

            $result = ( & "$eu_PathScript\eu_AlertaOcupacionDiscos.ps1" )
            Log "$scriptName -Service exec $eu_PathScript\eu_AlertaOcupacionDiscos.ps1: $result"
          }
        }
